...
for ((idx=0; idx<CHAIN_LENGTH; idx++)); do
	...
	if [[ "${DRY_RUN_FLAG,,}" == "true" ]]; then
		echo "[DRY RUN] Would run idx=$((idx + 1)) rank=${rank} lr=${lr} resume=${resume_run} omega=${CHAIN_OMEGAS[$idx]:-} switch_scale=${CHAIN_SWITCH_SCALES[$idx]:-}" >&2
		continue
	fi
	...
	job_name="galore_chain_r${rank}_lr${lr_tag}_hptrue"
