...
RUN_PREFIX=${RUN_PREFIX:-"icml2026-sparsity-local"}
...
run_python_config() {
	...
	PSEUDO_GRAD_TOP_K="${PSEUDO_GRAD_TOP_K:-}" \
...
total_planned=$(( CHAIN_LENGTH * ${#SYNC_FREQUENCIES[@]} * ${#SPARSITY_VALUES[@]} ))
echo "Planned GaLore chain (${total_planned} runs sweeping sync frequencies ${SYNC_FREQUENCIES_STR} and sparsity values ${SPARSITY_VALUES_STR}):"
for ((idx=0; idx<CHAIN_LENGTH; idx++)); do
	...
	for sync_freq in "${SYNC_FREQUENCIES[@]}"; do
		for sparsity_val in "${SPARSITY_VALUES[@]}"; do
			printf '    - sync_every=%4d sparsity=%s hp_switch=true qhm_outside=true\n' "${sync_freq}" "${sparsity_val}"
		done
	done
done
echo ""
...
for ((idx=0; idx<CHAIN_LENGTH; idx++)); do
	...
	for sync_freq in "${SYNC_FREQUENCIES[@]}"; do
	  for sparsity_val in "${SPARSITY_VALUES[@]}"; do
		...
		run_suffix=$(printf "idx%02d-r%d-lr%s-sync%s-%s" $((idx + 1)) "${rank}" "${lr_tag}" "${freq_tag}" "${sparsity_tag}")
		...
		job_name="galore_chain_r${rank}_lr${lr_tag}_sync${freq_tag}_${sparsity_tag}"
