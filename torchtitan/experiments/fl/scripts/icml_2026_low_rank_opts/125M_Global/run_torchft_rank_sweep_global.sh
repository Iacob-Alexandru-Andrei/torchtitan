...
RUN_PREFIX=${RUN_PREFIX:-"icml2026-125M-global-qhm"}
...
FULL_RANK_THRESHOLD=${FULL_RANK_THRESHOLD:-768}
...
for ((idx=0; idx<CHAIN_LENGTH; idx++)); do
	...
	# Require hyperparameter switch to be enabled for every run; skip otherwise.
	...
	if [[ "${DRY_RUN_FLAG,,}" == "true" ]]; then
		echo "[DRY RUN] Would run ${run_uuid} | chain=${chain_label} idx=$((idx + 1)) rank=${rank} lr=${lr} resume=${resume_run} omega=${CHAIN_OMEGAS[$idx]:-} switch_scale=${CHAIN_SWITCH_SCALES[$idx]:-}" >&2
		continue
	fi

	# Enforce per-run omega and switch-scale when generating configs.
