#!/usr/bin/env bash
# Launch Flux-schnell with DES-LOC param sync every 256 steps.
set -euo pipefail
...
CONFIG_FILE=${CONFIG_FILE:-"${SCRIPT_DIR}/flux_desloc_256.toml"}
...
RUN_PREFIX=${RUN_PREFIX:-"flux-desloc-256"}
