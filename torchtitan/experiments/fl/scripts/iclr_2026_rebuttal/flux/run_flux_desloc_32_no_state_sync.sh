#!/usr/bin/env bash
# Launch Flux-schnell with DES-LOC param sync 32, optimizer state sync disabled.
set -euo pipefail
...
CONFIG_FILE=${CONFIG_FILE:-"${SCRIPT_DIR}/flux_desloc_32_no_state_sync.toml"}
...
RUN_PREFIX=${RUN_PREFIX:-"flux-desloc-32-nosync"}
