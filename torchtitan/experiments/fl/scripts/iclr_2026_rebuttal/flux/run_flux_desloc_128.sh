#!/usr/bin/env bash
# Launch Flux-schnell with DES-LOC param sync every 128 steps.
set -euo pipefail
...
CONFIG_FILE=${CONFIG_FILE:-"${SCRIPT_DIR}/flux_desloc_128.toml"}
...
RUN_PREFIX=${RUN_PREFIX:-"flux-desloc-128"}
