#!/usr/bin/env bash
# MT-Dao sweep with quasi-hyperbolic AdamW (v=0.98) across worker counts and batch sizes.
set -euo pipefail
...
# Networking defaults (override as needed when launching the script).
...
for model_size in ${MODEL_SIZES}; do
  ...
  for global_bs in ${GLOBAL_BATCH_SIZES}; do
    for workers in ${WORKER_COUNTS}; do
      ...
      run_uuid="${RUN_PREFIX}-mtdao-${model_size}-w${workers}-gb${global_bs}-${timestamp}-${iteration}"
      ...
      echo "=== Launching MT-Dao run ${RUN_UUID} (model=${model_size}, workers=${workers}, global_bs=${global_bs}, local_bs=${local_bs}) ==="
