#!/usr/bin/env bash
# Launch the quasi-hyperbolic Muon TorchFT (\"mt_dao\") experiment with TorchFT replicas.
set -euo pipefail
...
CONFIG_FILE=${CONFIG_FILE:-"${SCRIPT_DIR}/base_desloc.toml"}
...
LIGHTHOUSE_PORT=${LIGHTHOUSE_PORT:-29840}
...
RUN_PREFIX=${RUN_PREFIX:-"iclr2026-136des16m"}
...
RDZV_BASE_PORT=${RDZV_BASE_PORT:-29240}
