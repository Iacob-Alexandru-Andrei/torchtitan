#!/usr/bin/env bash
# Sweep quasi-hyperbolic Muon TorchFT over (v, switch_scale) pairs.
set -euo pipefail

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
RUN_SCRIPT="${SCRIPT_DIR}/mt_dao_muon_torchft.sh"
...
for pair in "${PAIRS[@]}"; do
  ...
  echo "Launching TorchFT v=${v_value}, switch_scale=${scale_value}"
