#!/usr/bin/env bash
# Sweep AggMoAdamW AdeMaMix hyperparameters (new vs pairs + switch-scale) for the 16M flavor.
set -euo pipefail
...
Environment knobs:
  ...
  ADEMA_NEW_VS_PAIRS (space-separated \"a,b\" entries)
...
ADEMA_NEW_VS_PAIRS=${ADEMA_NEW_VS_PAIRS:-"0.4,0.5 0.3,0.6 0.2,0.7 0.1,0.8 0.0,0.9 0.45,0.5 0.35,0.6 0.25,0.7 0.15,0.8 0.05,0.9 0.0,0.95"}
...
validate_vs_pair() {
  local raw_pair=$1
  if [[ "${raw_pair}" != *,* ]]; then
    echo "Invalid vs pair '${raw_pair}' (expected format a,b)." >&2
    exit 1
  fi
  IFS=',' read -r first second <<< "${raw_pair}"
  if [[ -z "${first}" || -z "${second}" ]]; then
    echo "Invalid vs pair '${raw_pair}' (missing value)." >&2
    exit 1
  fi
  python3 - <<PY
first = float("${first}")
...
for pair in "${VS_PAIR_ARRAY[@]}"; do
  validate_vs_pair "${pair}"
done

TOTAL_RUNS=$(( ${#VS_PAIR_ARRAY[@]} * ${#SWITCH_SCALE_ARRAY[@]} ))
...
SWEEP_CONFIG_STRING="pairs=${ADEMA_NEW_VS_PAIRS}|switch=${ADEMA_SWITCH_SCALES}|lr=${BASE_LR}|switch_step=${WARMUP_SWITCH_STEP}|config=${CONFIG_FILE}"
...
print_run_plan() {
  ...
  for idx in "${!RUN_PLAN_INDICES[@]}"; do
    ...
    printf "%-10s %-10s %-18s %-18s\n" \
      "${combo_index_1}" \
      "${combo_index_0}" \
      "[${RUN_PLAN_V1[idx]},${RUN_PLAN_V2[idx]}]" \
...
submit_sbatch_job() {
  ...
  local new_vs_arg
  new_vs_arg=$(formatted_vs_arg "${v1}" "${v2}")
...
for idx in "${!RUN_PLAN_INDICES[@]}"; do
  ...
  run_uuid="${RUN_PREFIX}-${SWEEP_HASH}-v${v1_tag}-${v2_tag}-sw${scale_tag}-${timestamp_global}-idx${combination_index_zero}"
  ...
  if [[ "${USE_SBATCH}" == "true" ]]; then
    ...
    job_id=$(submit_sbatch_job "${run_uuid}" "${run_progress}" "${rdzv_endpoint}" "${lighthouse_url}" "${combination_index_zero}" "${v1}" "${v2}" "${switch_scale}" "${dependency_job}" "${chain_index}")
    if [[ -z "${job_id}" ]]; then
