#!/usr/bin/env bash
# Launch the 360M AdamW baseline with Mosaic's trainer (DDP) while disabling
# AdeMaMix's hyperparameter switch logic.
set -euo pipefail
...
ADAMW_SWITCH_SCALE=${ADAMW_SWITCH_SCALE:-1.0}

TIMESTAMP=$(date +"%Y%m%d-%H%M%S")
RUN_PREFIX=${RUN_PREFIX:-"iclr2026-adamw360M"}
