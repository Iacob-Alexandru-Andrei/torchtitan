#!/usr/bin/env bash
# Sweep Scion quasi-hyperbolic (ScionQH) momentum / v / switch-scale combinations.
set -euo pipefail

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
CONFIG_FILE=${CONFIG_FILE:-"${SCRIPT_DIR}/base.toml"}
TRAIN_MODULE=${TRAIN_MODULE:-"torchtitan.experiments.fl.train"}
RUN_PREFIX=${RUN_PREFIX:-"iclr2026-scion-qh"}
LOG_RANK=${LOG_RANK:-0}
...
usage() {
  cat <<'EOF'
Usage: run_scion_sweep.sh [OPTIONS] [-- extra training args]
...
VS_VALUES=${VS_VALUES:-"0.98 0.99 0.991 0.992 0.993 0.994 0.995 0.996 0.997 0.998 0.999 0.9995 0.9999 1.0"}
MOMENTUM_VALUES=${MOMENTUM_VALUES:-"0.90 0.92 0.94 0.96 0.98 0.99 0.995 1.0"}
SWITCH_SCALES=${SWITCH_SCALES:-"1.41421356237 2.82842712475 4 5.65685424949 8 11.31370849898 16"}

read -r -a VS_ARRAY <<< "${VS_VALUES}"
read -r -a MOMENTUM_ARRAY <<< "${MOMENTUM_VALUES}"
read -r -a SWITCH_SCALE_ARRAY <<< "${SWITCH_SCALES}"

(( ${#VS_ARRAY[@]} == 0 )) && { echo "VS_VALUES must contain at least one entry." >&2; exit 1; }
...
TOTAL_RUNS=$(( ${#VS_ARRAY[@]} * ${#MOMENTUM_ARRAY[@]} * ${#SWITCH_SCALE_ARRAY[@]} ))
...
SWEEP_CONFIG_STRING="vs=${VS_VALUES}|mom=${MOMENTUM_VALUES}|switch=${SWITCH_SCALES}|base_lr=${BASE_LR}|train_module=${TRAIN_MODULE}|config=${CONFIG_FILE}"
...
LIGHTHOUSE_BASE_PORT=${LIGHTHOUSE_BASE_PORT:-45100}
...
print_run_plan_table() {
  ...
  printf "%-10s %-10s %-10s %-14s %-18s\n" "Idx(1-based)" "Idx(0-based)" "v" "momentum" "switch_scale" >&2
  printf "%-10s %-10s %-10s %-14s %-18s\n" "----------" "----------" "--------" "--------------" "------------------" >&2
  for idx in "${!RUN_PLAN_INDICES[@]}"; do
    ...
    printf "%-10s %-10s %-10s %-14s %-18s\n" "${combo_index_1}" "${combo_index_0}" "${v_value}" "${momentum}" "${switch_scale}" >&2
  done
  echo "" >&2
}
...
echo "Run UUID: ${run_uuid} | Progress: ${run_progress} | v=${v_value} | momentum=${momentum} | switch_scale=${switch_scale}"
...
echo "Starting ScionQH sweep hash=${SWEEP_HASH} using ${EXECUTION_DESC}: ${SELECTED_RUNS}/${TOTAL_RUNS} run(s) selected (${FILTER_DESC})." >&2
...
for idx in "${!RUN_PLAN_INDICES[@]}"; do
  ...
  run_uuid="${RUN_PREFIX}-${SWEEP_HASH}-v${v_tag}-m${momentum_tag}-sw${scale_tag}-${timestamp_global}-idx${combination_index_zero}"
  run_progress="run ${combination_index}/${TOTAL_RUNS}"

  if [[ "${DRY_RUN}" == "true" ]]; then
    if [[ "${USE_SBATCH}" == "true" ]]; then
      echo "[DRY-RUN][SBATCH] ${run_uuid} (v=${v_value}, momentum=${momentum}, switch_scale=${switch_scale}) [${run_progress}]" >&2
    else
      echo "[DRY-RUN][LOCAL] ${run_uuid} (v=${v_value}, momentum=${momentum}, switch_scale=${switch_scale}) [${run_progress}]" >&2
    fi
    continue
  fi
  ...
  if [[ "${USE_SBATCH}" == "true" ]]; then
    ...
    job_id=$(submit_sbatch_job "${run_uuid}" "${run_progress}" "${rdzv_endpoint}" "${lighthouse_url}" "${combination_index_zero}" "${v_value}" "${momentum}" "${switch_scale}" "${dependency_job}" "${chain_index}")
    if [[ -z "${job_id}" ]]; then
    ...
    echo "[SBATCH] Submitted ${run_uuid} (${run_progress}) as job ${job_id} | v=${v_value}, momentum=${momentum}, switch_scale=${switch_scale}" >&2
    sleep 30
  else
    gpu_slot=$(wait_for_gpu)
    gpu_id=${GPU_ARRAY[gpu_slot]}
    echo "[GPU ${gpu_id}] Launching run ${run_uuid} (v=${v_value}, momentum=${momentum}, switch_scale=${switch_scale}) [${run_progress}]" >&2
